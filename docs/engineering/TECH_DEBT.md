...
| TD-012 | Medium | API scale | High-volume operational lists now have validated page/limit bounds and agent totals use page-scoped database aggregates; lower-growth reference lists and some report internals remain | Add cursor metadata, paginate remaining reference lists, and move report calculations fully into database aggregation. |
